...
def _load_saved_answers(client, session_id):
    ...
    f1_resp = _retry(lambda: (
        client.table("eval_format_1_timeline_v7")
        .select("*")
    ...
    f2_resp = _retry(lambda: (
        client.table("eval_format_2_tactics_v7")
        .select("*")
        .eq("session_id", session_id)
        .order("triple_index")
        .execute()
    ...
    f3_resp = _retry(lambda: (
        client.table("eval_format_3_boundaries_v7")
        .select("*")
        .eq("session_id", session_id)
        .order("option_index")
        .execute()
...
def _save_answers(session_id, f1_inputs, f2_inputs, f3_inputs,
    ...
    svc.table("eval_format_1_timeline_v7").delete().eq("session_id", session_id).execute()
    svc.table("eval_format_2_tactics_v7").delete().eq("session_id", session_id).execute()
    svc.table("eval_format_3_boundaries_v7").delete().eq("session_id", session_id).execute()
    ...
    if f1_inputs:
        ...
        svc.table("eval_format_1_timeline_v7").insert(f1_rows).execute()

    if f2_inputs:
        f2_rows = [{**common, **inp} for inp in f2_inputs]
        svc.table("eval_format_2_tactics_v7").insert(f2_rows).execute()

    if f3_inputs:
        f3_rows = [{**common, **inp} for inp in f3_inputs]
        svc.table("eval_format_3_boundaries_v7").insert(f3_rows).execute()
...
def render():
    ...
    # ── Fetch case data ──────────────────────────────────────────────────────
    case_resp = (
        client.table("synthetic_cases_v7")
        .select("*")
        .eq("id", case_id)
        .single()
        .execute()
    ...
    # ── V7 CHECKLIST FIELDS ───────────────────────────────────────────────────
    with st.expander("PN 3-Stage Lifecycle Checklist (Cool Down)", expanded=False):
        st.markdown(f"**Role Delineation Check:** {case.get('role_delineation_check', 'N/A')}")

        st.subheader("Stage 1: Entry & Triage")
        ...
        st.markdown(f"**Home vs LTC Goal:** {case.get('home_vs_ltc_goal', 'N/A')}")

        st.subheader("Stage 2: Maintenance")
        st.markdown(f"**Weekly Staff Update:** {case.get('weekly_staff_update', 'N/A')}")
        st.markdown(f"**V-Card & Flyer Status:** {case.get('v_card_flyer_status', 'N/A')}")

        st.subheader("Stage 3: Handoff")
        st.markdown(f"**Pre-DC Pulse Call:** {case.get('pre_dc_pulse_call', 'N/A')}")
    ...
    session_resp = _retry(lambda: (
        client.table("evaluation_sessions_v7")
    ...
    _save_answers(session_id, f1_inputs, f2_inputs, f3_inputs,
                  case_label, navigator_name)
    get_service_client().table("evaluation_sessions_v7").update({
