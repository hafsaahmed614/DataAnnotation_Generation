...
def _load_saved_answers(client, session_id):
    ...
    f1_resp = _retry(lambda: (
        client.table("eval_format_1_timeline_v12")
        .select("*")
    ...
    f2_resp = _retry(lambda: (
        client.table("eval_format_2_tactics_v12")
        .select("*")
        .eq("session_id", session_id)
        .order("triple_index")
        .execute()
    ...
    f3_resp = _retry(lambda: (
        client.table("eval_format_3_boundaries_v12")
        .select("*")
        .eq("session_id", session_id)
        .order("option_index")
        .execute()
...
def _save_answers(session_id, f1_inputs, f2_inputs, f3_inputs,
    ...
    svc.table("eval_format_1_timeline_v12").delete().eq("session_id", session_id).execute()
    svc.table("eval_format_2_tactics_v12").delete().eq("session_id", session_id).execute()
    svc.table("eval_format_3_boundaries_v12").delete().eq("session_id", session_id).execute()
    ...
    if f1_inputs:
        ...
        svc.table("eval_format_1_timeline_v12").insert(f1_rows).execute()

    if f2_inputs:
        f2_rows = [{**common, **inp} for inp in f2_inputs]
        svc.table("eval_format_2_tactics_v12").insert(f2_rows).execute()

    if f3_inputs:
        f3_rows = [{**common, **inp} for inp in f3_inputs]
        svc.table("eval_format_3_boundaries_v12").insert(f3_rows).execute()
...
def render():
    ...
    # ── Fetch case data ──────────────────────────────────────────────────────
    case_resp = (
        client.table("synthetic_cases_v12")
        .select("*")
        .eq("id", case_id)
        .single()
        .execute()
    ...
    session_resp = _retry(lambda: (
        client.table("evaluation_sessions_v12")
    ...
    _save_answers(session_id, f1_inputs, f2_inputs, f3_inputs,
                  case_label, navigator_name)
    get_service_client().table("evaluation_sessions_v12").update({
